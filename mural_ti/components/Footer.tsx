...
const Footer: React.FC = () => {
  const quickLinks = [
    { label: 'Avisos Coordenação', href: ViewType.AVISOS },
    { label: 'Mercado de TI', href: ViewType.PROFISSIONAIS },
    { label: 'Carreira & Vagas', href: ViewType.ESTAGIOS },
    { label: 'Como Desenvolver', href: ViewType.DESENVOLVIMENTO },
    { label: 'Novo Currículo 2026', href: ViewType.CURRICULO },
  ...
  return (
    <footer className="bg-[#020617] text-slate-400 pt-12 pb-4 border-t border-white/5 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-4">
            ...
            <div className="flex gap-3">
              {[
                ...
                <a 
                  ...
                  className={`w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center transition-all duration-300 border border-white/5 group ${social.color}`}
                >
          ...
          {/* Navigation Column */}
          <div>
            <h4 className="text-white font-black uppercase text-[9px] tracking-[0.4em] mb-8 italic flex items-center gap-3">
              <span className="w-1.5 h-1.5 bg-estacio-cyan rounded-full"></span> MAPA DO HUB
            </h4>
            <ul className="grid grid-cols-1 gap-3 text-[10px] font-black uppercase tracking-widest">
              {quickLinks.map((link) => (
                <li key={link.href}>
                  <a 
                    href={link.href} 
                    onClick={(e) => handleLinkClick(e, link.href)}
                    className="hover:text-estacio-cyan transition-all flex items-center gap-3 group"
                  >
                    <span className="w-1 h-1 rounded-full bg-slate-800 group-hover:bg-estacio-cyan transition-all"></span>
                    {link.label}
                  </a>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-4 text-[10px] font-black uppercase tracking-widest">
              {[
                ...
                <li key={i}>
                  <a href={item.href} onClick={(e) => handleLinkClick(e, item.href)} className="hover:text-white transition-all flex items-center gap-3 group">
                    <div className={`w-8 h-8 ${item.bg} rounded-lg flex items-center justify-center ${item.color} group-hover:scale-110 transition-all border border-white/5`}>
                      <item.icon size={14} />
                    </div>
                    {item.label}
                  </a>
          ...
          <div className="lg:pl-6">
            ...
            <div className="bg-gradient-to-br from-white/5 to-transparent p-6 rounded-[2.5rem] border border-white/10 backdrop-blur-xl group hover:border-estacio-cyan/30 transition-all">
              <p className="text-[8px] font-black uppercase tracking-[0.3em] text-slate-500 mb-1">Coordenação</p>
              <p className="text-base font-black text-white italic mb-4 tracking-tighter uppercase leading-tight">
                ANTONIO <span className="text-estacio-cyan">CANDIDO</span>
              </p>
              
              <a 
                href="mailto:antonio.filho@estacio.br"
                className="flex items-center justify-center gap-2 py-3 bg-estacio-navy text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-estacio-cyan hover:text-estacio-navy transition-all shadow-xl shadow-black/20"
              >
                CONTATAR AGORA
              </a>
            </div>
        ...
        <div className="pt-4 border-t border-white/5 flex flex-col items-center">
          ...
          <div className="max-w-4xl text-center">
            <p className="text-[9px] font-bold text-slate-600 leading-tight italic uppercase tracking-normal opacity-60">
